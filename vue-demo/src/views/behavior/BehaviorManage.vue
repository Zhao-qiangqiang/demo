<template>
    <div>
        <!--搜索界面-->
        <BehaviorSearch :search="search" :so="so"></BehaviorSearch>
        <!--按钮-->
        ...
        <div class="form">
            <el-table :data="pageList.items" ref="listTable" border style="width: 100%;">
                ...
                <el-table-column label="来源">
                    <template
                        slot-scope="scope"
                    >{{scope.row.platform==="miniapp"?"小程序":scope.row.platform==="app"?"APP":"未知"}}</template>
                </el-table-column>
...
<script>
import Api from "api";
import BehaviorSearch from "./BehaviorSearch";
...
export default {
    ...
    methods: {
         ...
         */
        getPageList_api() {
            Api.behavior.getPageList(this.so).then(res => {
                this.pageList = res;
            });
        },
