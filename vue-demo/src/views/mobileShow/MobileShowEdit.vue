...
export default {
    ...
    methods: {
         ...
         */
        get_api(selectedId) {
            Api.mobileShow.getById({ id: selectedId }).then(res => {
                this.vo = res;
                this.vo.openTime = new Date(
                    this.vo.openTime.replace(/-/g, "/")
                );
                this.vo.endTime = new Date(this.vo.endTime.replace(/-/g, "/"));
                this.dynamicValidateForm.domains = this.vo.cityList;
            });
        },
        /**
         * 校验表单
